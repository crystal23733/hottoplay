...
// Analyzer는 메가밀리언 번호 통계 분석을 담당하는 구조체입니다.
type Analyzer struct {
	draws []models.MegaMillionsDraw
}

// NewAnalyzer는 새로운 Analyzer 인스턴스를 생성합니다.
func NewAnalyzer(draws []models.MegaMillionsDraw) *Analyzer {
	return &Analyzer{
		draws: draws,
	}
}

// parseDate는 "April 11, 2025" 형식의 문자열을 time.Time으로 변환합니다.
...
func (a *Analyzer) GetNumberStatistics(numbers []int) []models.NumberStatistics {
	...
	for i, num := range numbers {
		...
		var lastMegaBallTime time.Time
		...
		for _, draw := range a.draws {
			...
			// 메가볼로 사용된 경우 확인
			if draw.MegaBall == num {
				stat.MegaBallCount++
				currentTime, _ := parseDate(draw.Date)
				if currentTime.After(lastMegaBallTime) {
					lastMegaBallTime = currentTime
					stat.LastMegaBallDate = draw.Date
				}
			}
		}
...
func (a *Analyzer) GetCombinationStatistics(numbers []int) *models.CombinationStatistics {
	...
	for _, draw := range a.draws {
		...
		copy(drawNumbers, draw.WhiteNumbers)
