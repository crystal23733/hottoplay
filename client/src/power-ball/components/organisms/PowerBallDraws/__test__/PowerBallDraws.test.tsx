...
import PowerBallDraws from '../PowerBallDraws';
import usePowerBallDrawList from '@/power-ball/hooks/usePowerBallDrawList';
import { useToast } from '@/common/hooks/use-toast';

// 훅 모킹
jest.mock('@/power-ball/hooks/usePowerBallDrawList');
...
describe('PowerBallDraws 컴포넌트', () => {
  ...
  beforeEach(() => {
    ...
    // usePowerBallDrawList 훅 모킹
    (usePowerBallDrawList as jest.Mock).mockReturnValue({
      data: mockData,
      loading: false,
      error: null,
      page: 1,
  ...
  it('컴포넌트가 올바르게 렌더링되어야 합니다', () => {
    render(<PowerBallDraws />);

    // 헤더가 있는지 확인
    expect(screen.getByText('Powerball Drawing Results')).toBeInTheDocument();
