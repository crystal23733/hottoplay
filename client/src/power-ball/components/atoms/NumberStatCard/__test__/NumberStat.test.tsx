...
describe('NumberStatCard', () => {
  const mockStat = {
    ...
    last_power_ball_date: 'Mon, Dec 30, 2024',
  };
  ...
  it('shows total appearances', () => {
    render(<NumberStatCard stat={mockStat} />);
    const totalAppearances = mockStat.white_ball_count + mockStat.power_ball_count;
    expect(
  ...
  it('displays powerball statistics', () => {
    ...
    expect(
      screen.getByText(
        `${MESSAGES.POWER_BALL.APPEARED} ${mockStat.power_ball_count} ${MESSAGES.POWER_BALL.TIMES}`
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(`${MESSAGES.POWER_BALL.LAST_DRAWN}: ${mockStat.last_power_ball_date}`)
    ).toBeInTheDocument();
  });
  ...
  it('handles missing dates gracefully', () => {
    ...
    expect(
      screen.queryByText(new RegExp(`${MESSAGES.POWER_BALL.LAST_DRAWN}:`))
    ).not.toBeInTheDocument();
  });
