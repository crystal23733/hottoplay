...
import MegaMillionsDrawDetailProps from './MegaMillionsDrawDetail.types';
...
/**
 * 메가밀리언 추첨 결과 상세 페이지 컴포넌트
 * @param {MegaMillionsDrawDetailProps} props - 메가밀리언 추첨 결과 상세 페이지 컴포넌트 속성
 * @param {string} props.date - 추첨 날짜
 * @returns {React.ReactNode} 파워볼 추첨 결과 상세 페이지 컴포넌트
 */
const MegaMillionsDrawDetail: React.FC<MegaMillionsDrawDetailProps> = ({ date }) => {
  const { data, loading, error } = useMegaMillionsDrawDetail(date);
  ...
  return (
    <div className="container max-w-4xl mx-auto py-8 px-4">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">Mega Millions Result: {data.draw_date}</h1>
        <p className="text-muted-foreground">Multiplier: {data.mega_plier || 'None'}</p>
      </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              {data.cash_option && (
                <div>
                  <span className="font-medium">Cash Value:</span> <span>{data.cash_option}</span>
                </div>
              )}

              <div>
                <span className="font-medium">Jackpot Winners:</span>{' '}
                <span>{data.jackpot_winners ? data.jackpot_winners + ' winners' : 'None'}</span>
              </div>
            </div>
          </CardContent>
        </Card>
...
export default MegaMillionsDrawDetail;
